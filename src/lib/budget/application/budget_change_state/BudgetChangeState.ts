...
export class BudgetChangeState {
  ...
  async Run(id: number, amount: number, state: string, createdAt: string): Promise<BudgetDto> {
    ...
    const budget: Budget | null = await this._budgetRepository.ChangeState(
      new Budget(
    ...
    if (!budget) throw new Error('error changing the budget state')
